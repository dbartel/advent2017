...
(defun split-seq (seq delimiter)
  (let ((delimiterp (lambda (c) (char= c delimiter))))
...
(defun validate-passphrase-no-duplicates (phrase)
  (let ((tokens (split-seq phrase #\Space)))
    (not (find :invalid (mapcar (lambda (w)
				  (if (> (length (remove-if-not (lambda (c) (string= c w)) tokens)) 1)
				      :invalid
				      :valid)) tokens)))))

(defun validate-passphrase-no-anagrams (phrase)
  ;; 1. Alphabetize characters
  ;; 2. Search
  (let* ((tokens (split-seq phrase #\Space))
	 (anagrams (mapcar (lambda (c) (sort c #'char-lessp)) tokens))
	 (candidates (mapcar (lambda (c) (remove-if-not (lambda (w) (string= c w)) anagrams) ) anagrams)))
    (not (find :invalid (mapcar (lambda (w) (if (> (length w) 1) :invalid :valid)) candidates)))))

(defun count-valid-passphrases (phrases validation-fn)
  (length (remove-if-not (lambda (p) p) (mapcar validation-fn phrases))))


(defparameter input (split-seq "oaoe rxeq vssdqtu xrk cjv yaoqp loo
...
(format t "Part One: ~a~&" (count-valid-passphrases input 'validate-passphrase-no-duplicates))
(format t "Part Two: ~a~&" (count-valid-passphrases input 'validate-passphrase-no-anagrams))
